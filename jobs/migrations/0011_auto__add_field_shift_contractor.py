...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding field 'Shift.contractor'
        db.add_column('jobs_shift', 'contractor',
                      self.gf('django.db.models.fields.related.ForeignKey')(related_name='shifts', null=True, to=orm['users.Contractor']),
                      keep_default=False)

        # Adding M2M table for field standby_contractors on 'Shift'
        db.create_table('jobs_shift_standby_contractors', (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('shift', models.ForeignKey(orm['jobs.shift'], null=False)),
            ('contractor', models.ForeignKey(orm['users.contractor'], null=False))
        ))
        db.create_unique('jobs_shift_standby_contractors', ['shift_id', 'contractor_id'])

    def backwards(self, orm):
        # Deleting field 'Shift.contractor'
        db.delete_column('jobs_shift', 'contractor_id')

        # Removing M2M table for field standby_contractors on 'Shift'
        db.delete_table('jobs_shift_standby_contractors')
